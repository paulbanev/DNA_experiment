...
def export_to_excel(results):
    ...
    for metric_name, data in results.items():
        if metric_name == "eigenvector matrix":
            # Stack all V matrices into one DataFrame with Run labels
            all_Vs = []
            for i, V in enumerate(data):
                df = pd.DataFrame(V)
                df.insert(0, "Row", range(1, V.shape[0] + 1))  # optional: label rows
                df.insert(0, "Run", f"Run {i}")
                all_Vs.append(df)
                all_Vs.append(pd.DataFrame([[""] * df.shape[1]]))  # blank row between runs
